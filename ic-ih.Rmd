...
1. Envie o arquivo [em mensagem privada]{.hl} para o professor, via Telegram.
...
1. [Registre]{.hl}, para cada pessoa,
   ...
   * A [distância, em centímetros]{.hl}, com $1$ casa decimal, [entre a ponta do dedo mínimo e a ponta do polegar]{.hl} (ambos da mão esquerda), com a mão mais aberta possível --- como se a pessoa fosse tocar, ao mesmo tempo, só com a mão esquerda, duas teclas muito afastadas de um piano.

1. [Registre]{.hl} os mesmos dados [para você mesmo]{.hl}.

1. [Gere o arquivo `razao.csv`]{.hl} com os resultados. 

   O arquivo terá $6$ linhas e [$4$ colunas]{.hl}:
...
1. [Registre]{.hl}, para cada pessoa,
   ...
   * O [peso em quilogramas]{.hl}, com $3$ casas decimais.
   
1. [Registre]{.hl} os mesmos dados [para você mesmo]{.hl}.

1. [Gere o arquivo `peso.csv`]{.hl} com os resultados. 

   O arquivo terá $6$ linhas e [$2$ colunas]{.hl}:

   * `sexo`: `M` ou `F`.
   
   * `peso`: um valor de ponto flutuante [com $3$ casas decimais]{.hl}.
